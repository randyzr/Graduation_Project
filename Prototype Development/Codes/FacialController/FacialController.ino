...
// =============================================================================
// FACIAL CONTROLLER: AUTONOMOUS FACE DETECTION AND RECOGNITION
// =============================================================================
// This ESP32-S3 continuously captures camera frames. FacialWebServer.cpp performs
// detection/recognition in a background FreeRTOS task, while this file sends the
// latest result to the MainController through ESP-NOW. Enrollment is the only
// camera action that must be requested by the HMI.

// ----- Communication and shared recognition state ----------------------------
...
volatile bool deleteFacesRequested = false;
volatile uint32_t lastDeleteFacesCommand = 0;
...
void onEspNowReceive(const esp_now_recv_info_t *info, const uint8_t *data,
  ...
  if (message.magic != ESD_MESSAGE_MAGIC ||
      message.version != ESD_PROTOCOL_VERSION) return;
  if (message.type == MESSAGE_ENROLL_FACE &&
      message.sequence != lastEnrollmentCommand) {
    lastEnrollmentCommand = message.sequence;
    enrollmentRequested = true;
  } else if (message.type == MESSAGE_DELETE_ALL_FACES &&
             message.sequence != lastDeleteFacesCommand) {
    lastDeleteFacesCommand = message.sequence;
    deleteFacesRequested = true;
  }
}

// Called by the background recognition task. Reading and clearing this flag in
// one place prevents one enrollment request from being processed repeatedly.
...
bool consumeDeleteFacesRequest() {
  if (!deleteFacesRequested) return false;
  deleteFacesRequested = false;
  return true;
}
...
// ----- Status messages sent to MainController --------------------------------
...
// Called by FacialWebServer.cpp after processing each streamed camera frame.
...
// ----- Camera and radio initialization ---------------------------------------
...
void setup() {
  ...
  // FacialWebServer.cpp then performs the required 90-degree portrait rotation.
  s->set_vflip(s, 1);
  s->set_hmirror(s, 1);
  ...
  // From this point onward the camera works even while the HMI is on Welcome.
...
// ----- Arduino main loop -----------------------------------------------------
// Recognition itself runs in a background task. This loop only sends a regular
// heartbeat so MainController can tell that the camera is still connected.
